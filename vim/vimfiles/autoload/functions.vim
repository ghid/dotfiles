...
function! functions#Statusline()
	...
	let statusline=""
	let statusline.="%1*\ %{toupper(g:currentmode[mode()])}\ "
	let statusline.="%#StatusLine#\ %{functions#GitBranch()}\ "
	let statusline.="\ %{functions#DiagnosticStatus()}"
	let statusline.="\ %{functions#SpellCheck()}\ "
	let statusline.="%="
	if winwidth("%") > 75
		let statusline.="\ Ln\ %l,\ Col\ %v\ "
		let statusline.="\ Sp\ %{&tabstop}\ "
		let statusline.="\ %{''.toupper(&fenc!=''?&fenc:&enc).''}\ "
		let statusline.="\ %{(&bomb?'':'')}\ "
		let statusline.="\ %{(&ff==#'dos'?'CRLF':&ff==#'unix'?'LF':&ff==#'mac'?'CR':&ff)}\ "
	else
		let statusline.="\ L\ %l\ C\ %v\ "
	endif
	let statusline.="\ %Y\ "
	let statusline.="\ %{functions#FileSize()}\ "
	let statusline.="%{(&readonly?'':'\ ')}\ "
	let statusline.="%{(&modified?'●\ ':'\ ')}\ "
	return statusline
endfunction

function! functions#StatuslineInactive()
	let statusline=""
	let statusline.="%#StatusLineNC#\ %f\ "
	let statusline.="%="
	let statusline.="\ Ln\ %l,\ Col\ %v\ "
	let statusline.="\ Spaces:\ %{&tabstop}\ "
	let statusline.="\ %{''.toupper(&fenc!=''?&fenc:&enc).''}\ "
	let statusline.="\ %{(&ff==#'dos'?'CRLF':&ff==#'unix'?'LF':&ff==#'mac'?'CR':&ff)}\ "
	let statusline.="\ %Y\ "
	let statusline.="\ %{functions#FileSize()}\ "
...
function! functions#QuickRunKotlin(bufName) abort
	echo bufName
endfunction
